...
//Punto de fuga, tolerancia de lineas verticales/horizontales
#define ToleranciaH 5
#define ToleranciaV 30

#define MAX_LINES 150

#define ANG_THRES 30

#define cann1_THRES1 20
#define cann1_THRES2 200
#define cann1_SIZE 3

#define cann2_THRES1 180
#define cann2_THRES2 120
#define cann2_SIZE 3

#define HOUGH_INIT 70

#define MEDIA_THRES 5

//Parametros de calibracion del bucle de control
#define ALISADO_Y 4
#define ALISADO_Z 4
#define K_Y 5
#define K_Z 5
...
Point anterior = Point(0,0);

//Guarda el historial de desviaciones en y, z
double desv_y = 0;
double desv_z = 0;
...
//Funcion pintar puntos
void pintarPuntos(Mat fondo, Vector<Point> puntos);

//Funcion pintar linea
Mat pintarLineas(Mat fondo, Vector <Vec2f> lineas);

//Funcion intersección
vector<Point> intersec(vector<Vec2f> lineas);
 
//Busca el centro geometrico
Point centroGeometrico(vector<Point> puntos);

//Funcion limpiarLineasHV
vector<Vec2f> limpiarLineasHV(vector<Vec2f> sucias);
//Funcion bucle en y
void giro_y(double desv_centro, double desv_fuga , int rows);
//Funcion bucle en z
void giro_z(double desv_fuga, int rows);
list <string> nombres;
...
public:
    ...
    : it_(nh_)
  {
    ...
    offset_pub_ = nh_.advertise<geometry_msgs::Point>("/ise_vision/offset", 1);    
...
//	std::cout << "se abre";
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	Mat detected_edges, detected_edges2, canny_edges, canny_edges2;
	blur(cv_ptr->image, detected_edges, Size(3,3) );	//Primera distorsión
	blur(cv_ptr->image, detected_edges2, Size(2,2) );
 	Canny(detected_edges,canny_edges, cann1_THRES1, cann1_THRES2, cann1_SIZE);	//Obtenemos las líneas principales de la imágen
	Canny(cv_ptr->image,canny_edges2, cann1_THRES2, cann2_THRES2, cann2_SIZE);	//Obtenemos las líneas principales de la imágen
 	//Detección de lineas rectas
	vector<Vec2f> lines;
	int hough = HOUGH_INIT;
	HoughLines(canny_edges2, lines, 1, CV_PI/180, hough, 0, 0);
	...
	//Pintar lineas rectas
  	//pintarLineas(cdst, lines, "Detección de lineas Bruta");
	//Eliminar lineas Horizontales y verticales
	lines = limpiarLineasHV(lines);
	//Pintar lineas rectas limpias
	//pintarLineas(cdst, lines, "Detección de lineas Limpia");
	//Cruces de lineas
	Point puntoFuga;
	...
	if(lines.size() > 1){
		intersections = intersec(lines);
	//pintar puntos
 	//pintarPuntos(cdst, intersections, "intersecciones enontradas");
	//calcular centro geometrico, punto de fuga
  		puntoFuga = centroGeometrico(intersections); 
	}
	else{
		puntoFuga.x = 0;
		puntoFuga.y = 0;
	...
	if( puntoFuga != Point(0,0)){
		anterior.x = (puntoFuga.x + ALIS_FUGA*anterior.x)/(ALIS_FUGA+1);
		anterior.y = (puntoFuga.y + ALIS_FUGA*anterior.y)/(ALIS_FUGA+1);
	}
	puntoFuga = anterior;
	fuga_out[0] = puntoFuga.x - (int)cdst.cols/2;
	fuga_out[1] = puntoFuga.y - (int)cdst.rows/2;
	...
	Point medio; 
	int medio_out[2];
	//Variables que van de -1 a 1 e indican si hay que girar en y o z. -1 = todo izquierda, 1 = todo derecha, 0 = seguir recto
	double gira_y = 0;
	double gira_z = 0;
	int code_vec_size;
	...
	//ELSE ERRORRRRRR*/
	for( int s = 0; s < intersections.size(); s++){
		circle( cdst, intersections[s], 8, Scalar(100,100,100), 1);
	}
	//Pintamos lineas
	pintarLineas(cdst, lines);
	//pintar punto de fuga
  	circle( cdst, puntoFuga, 8, Scalar(200,100,0), 3);
	//Sacamos por pantalla las imágenes que necesitemos ver
	imshow("Canny", canny_edges2);
	//imshow("Detected", detected_edges);
	imshow("Final", cdst);
	while(waitKey(10)>=0){break;}
	//imshow("Lineas detectadas", canny_edges);
	//imshow("Contornos", detected_edges);
 	//OJOOOOO
	//Hay que enviar code_final, medio_out y areaContEsquina
	giro_z(fuga_out[0], cdst.rows);	
	giro_y(medio_out[0], fuga_out[0], cdst.rows);
	gira_y = desv_y * K_Y;
	gira_z = desv_z * K_Z;
	cout << "Gira en y: " << gira_y << "    Gira en z: " << gira_z << endl;
...
void lectura_lista(){
	// Abre un fichero de entrada
  	std::ifstream In("/home/miguel/bebop_WS/src/ise_vision/src/Codigos.txt");
...
//Funcion pintar puntos_____________________________________________________________
void pintarPuntos(Mat fondo, Vector<Point> puntos){
 for( size_t i = 0; i < puntos.size(); i++ )
  {
  circle(fondo, puntos[i], 3, Scalar(255,0,0), 1);
  }
};

//Funcion pintar linea_____________________________________________________________
Mat pintarLineas(Mat fondo, Vector <Vec2f> lineas){
 for( size_t i = 0; i < lineas.size(); i++ )
  {
     float rho = lineas[i][0], theta = lineas[i][1];
     Point pt1, pt2;
     ...
     line( fondo, pt1, pt2, Scalar(0,0,255), 1, CV_AA);
  }
  return fondo;
};

//_______________________________________________________________________________
//Funcion intersección______________________________________________________________
vector<Point> intersec(vector<Vec2f> lineas){
vector<Point> intersecciones;
 for( size_t i = 0; i < lineas.size(); i++ )
 {
 	if(lineas.size()<=1){ 
		...
		float rho1 = lineas[i][0], theta1 = lineas[i][1];
		for( size_t j = i+1; j < lineas.size(); j++ )//no compararse consigo misma ni con las anteriores
 		{	
			Point interseccion;
			float rho2 = lineas[j][0], theta2 = lineas[j][1];
			//Cálculo de la intersección entre la linea i con la linea j.
			//No tenemos en cuenta aquellas lineas con  angulo entre ellas pequeño.
			if( abs(theta1-theta2) > (CV_PI/180)*ANG_THRES && abs(cos(theta1)) >0.01 && abs(cos(theta2))> 0.01 ){
				interseccion.x = (rho2/sin(theta2)-rho1/sin(theta1))/(1/tan(theta2)-1/tan(theta1));
				interseccion.y = (rho2/cos(theta2)-rho1/cos(theta1))/(tan(theta2)-tan(theta1));
				intersecciones.push_back(interseccion);
			}
		}
	}
...
//Función centroGeometrico______________________________________________________________
Point centroGeometrico(vector<Point> puntos){
 Point centro;
 int distMin = 0;
 int pos = 0;
 ...
 if (puntos.size()>1){
 	for( size_t i = 0; i < puntos.size(); i++ )
 	{
 		for( size_t j = 0; j < puntos.size(); j++ ){
			if(j != i){
				//int distancia = sqrt((puntos[i].x - puntos[j].x)*(puntos[i].x - puntos[j].x) + (puntos[i].y - puntos[j].y)*(puntos[i].y - puntos[j].y));	
				distanciaTot += 10;//distancia;
			}
		}
		if( distanciaTot <= distMin){
			distMin = distanciaTot;
			pos = i;
		}	
 	}
 	centro.x = puntos[pos].x;
 	centro.y = puntos[pos].y;
 	return centro;
 	}
 	else if(puntos.size() <= 1) return Point(0,0);
};

//_________________________________________________________________________________
//Funcion limpiarLineasHV___________________________________________________________
vector<Vec2f> limpiarLineasHV(vector<Vec2f> sucias){
 vector<Vec2f> limpias; 
if(sucias.size()>1){
 for( size_t i = 0; i < sucias.size(); i++ )
 {
	float theta = (180/CV_PI)*sucias[i][1];//void push_back (const value_type& val);
	//se es menos restrictivo en las lineas horizontales
	if(abs(theta) > (ToleranciaV) && abs(theta-90) > (ToleranciaH) && abs(theta-180) > ToleranciaV && abs(theta-270) > (ToleranciaH)) 
		limpias.push_back(sucias[i]);
 }
 return limpias;
 }
 else if(sucias.size()) return sucias;
 else return limpias; 
};

// Funcion bucle de control del giro en Z
void giro_z(double desv_fuga, int rows) {
double sumar = 0;
desv_fuga = 2*desv_fuga/((double)rows);

if (((desv_fuga - desv_z) < AMOR_Z) && ((desv_fuga - desv_z) > -AMOR_Z)) sumar = desv_fuga - desv_z;
else {	
	// Si la diferencia entre el punto de fuga y la variación almacenada es mayor de un 20%, solo se incrementa un 20% para que no varíe muy rápido	
	if ((desv_fuga - desv_z) > AMOR_Z) sumar = AMOR_Z;
	if ((desv_fuga - desv_z) < -AMOR_Z) sumar = -AMOR_Z;
}

// Modificar el numero que multiplica a desv_z  y el divisor para variar el alisado. Reducirlos para hacerlo más rápido, aumentarlos para hacerlo mas lento. divisor = nº que multiplica + 1
//desv_z = (sumar + ALISADO_Z*desv_z)/(ALISADO_Z + 1);
desv_z = (sumar + ALISADO_Z*desv_z)/(ALISADO_Z + 1);
return;

};

// Función bucle de control en y

void giro_y(double desv_centro, double desv_fuga, int rows) {

desv_fuga = desv_fuga/(rows/2);
desv_centro = desv_centro/(rows/2);
double sumar = 0;
double desviacion = 0;
desviacion = desv_fuga - desv_centro;

if (((desviacion - desv_y) < AMOR_Y) && ((desviacion - desv_y) > -AMOR_Y)) sumar = desviacion;
else {	
	// Si la diferencia entre el punto de fuga y la variación almacenada es mayor de un 20%, solo se incrementa un 20% para que no varíe muy rápido	
	if ((desviacion - desv_y) > AMOR_Y) sumar = desv_y + AMOR_Y;
	if ((desviacion - desv_y) < -AMOR_Y) sumar = desv_y - AMOR_Y;
}

// Modificar el numero que multiplica a desv_y  y el divisor para variar el alisado. Reducirlos para hacerlo más rápido, aumentarlos para hacerlo mas lento. divisor = nº que multiplica + 1
desv_y = (sumar + ALISADO_Y*desv_y)/(ALISADO_Y + 1);
